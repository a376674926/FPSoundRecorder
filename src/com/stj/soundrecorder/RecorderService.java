...
public class RecorderService extends Service implements MediaRecorder.OnErrorListener {
    ...
    public final static int ACTION_SHOW_RECORDING_NOTIFICATION = 5;

    public final static int ACTION_SHOW_STOP_RECORD_NOTIFICATION = 6;
    ...
    public final static String FILE_SIZE_LIMIT_STOP = "file_size_limit_stop";
                ...
                switch (bundle.getInt(ACTION_NAME, ACTION_INVALID)) {
                    case ACTION_START_RECORDING:
                        localStartRecording(bundle.getInt(ACTION_PARAM_FORMAT),
                                bundle.getString(ACTION_PARAM_SAMPLE_FILE),
                    ...
                    case ACTION_DISABLE_MONITOR_REMAIN_TIME:
                        ...
                        break;
                    case ACTION_SHOW_RECORDING_NOTIFICATION:
                        showRecordingNotification();
                        break;
                    case ACTION_SHOW_STOP_RECORD_NOTIFICATION:
    ...
    private void updateRemainingTime() {
        ...
        if (t <= 0) {
            Intent intent = new Intent(RECORDER_SERVICE_BROADCAST_NAME);
            intent.putExtra(RECORDER_SERVICE_BROADCAST_STATE, mRecorder != null);
            intent.putExtra(FILE_SIZE_LIMIT_STOP, true);
            sendBroadcast(intent);
            return;
        } else if (t <= 1800
    ...
    public static void startRecording(Context context, int outputfileformat, String path,
        ...
        intent.putExtra(ACTION_PARAM_SAMPLE_FILE, path);
        intent.putExtra(ACTION_PARAM_HIGH_QUALITY, highQuality);
        intent.putExtra(ACTION_PARAM_MAX_FILE_SIZE, maxFileSize);
        context.startService(intent);
    }

    public static void showNotification(Context context, int state) {
        Intent intent = new Intent(context, RecorderService.class);
        switch (state) {
            case ACTION_START_RECORDING:
                intent.putExtra(ACTION_NAME, ACTION_SHOW_RECORDING_NOTIFICATION);
                context.startService(intent);
                break;
            case ACTION_STOP_RECORDING:
                intent.putExtra(ACTION_NAME, ACTION_SHOW_STOP_RECORD_NOTIFICATION);
                context.startService(intent);
                break;
