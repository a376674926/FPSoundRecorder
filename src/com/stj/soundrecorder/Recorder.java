...
public class Recorder implements OnCompletionListener, OnErrorListener, OnSharedPreferenceChangeListener {
    ...
    private String mPrefStoragePath = null;

    private boolean mIsLimitSizeStop = false;

    public Recorder(Context context) {
        mContext = context;
        File sampleDir = new File(SoundRecorderPreferenceActivity.getPrefStoragePath(mContext)
                + SAMPLE_DEFAULT_DIR);
        if (!sampleDir.exists()) {
            sampleDir.mkdirs();
        }
    ...
    public void delete() {
        ...
        if (mSampleFile != null) {
            mContext.getContentResolver().delete(
                    ...
                    new String[] { mSampleFile.getPath() });
            mSampleFile.delete();
        }
    ...
    public void startRecording(int outputfileformat, String name, String extension,
        ...
        if (mSampleFile == null) {
            ...
            if (mSampleFile.getAbsolutePath().startsWith("/system")) {
                Log.w(TAG, "mSampleFile=" + mSampleFile.getAbsolutePath());
                mSampleFile = null;
                return;
            }
        }
    ...
    public void startPlayback(float percentage) {
        if (state() == PLAYING_PAUSED_STATE && !mIsLimitSizeStop) {
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        if (key == SoundRecorderPreferenceActivity.KEY_FILE_STORAGE_PATH) {
            File sampleDir = new File(SoundRecorderPreferenceActivity.getPrefStoragePath(mContext)
                    + SAMPLE_DEFAULT_DIR);
            if (!sampleDir.exists()) {
                sampleDir.mkdirs();
    ...
    public void setLimitSizeStop(boolean limitStop){
        mIsLimitSizeStop = limitStop;
    }
}
